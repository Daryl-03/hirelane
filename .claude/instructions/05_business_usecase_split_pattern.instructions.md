...
# Instructions Copilot - Business Logic & Use Cases Strategy (Next.js)
...
- **Inversion de Dépendance via Constructeur :** Le Use Case ne dépend JAMAIS d'une implémentation technique (comme `PrismaTodoRepository`). Il dépend d'un **Port** qu'il reçoit via son constructeur.
- **Agnosticisme :** Le Use Case ne doit **jamais** utiliser `'use server'`, `'use client'`, ou des imports React/Next.js. Il reste une classe TypeScript pure. Son injection sera configurée dans la couche Infrastructure ou Présentation via des factory functions ou Context Providers.

## 2. Règle d'Or des Ports (Classes Abstraites pour l'Injection)
C'est le domaine qui dicte ses besoins à l'infrastructure, et non l'inverse.

- **Définition :** Les "Ports" (Repositories, Notifiers, API clients) doivent être définis dans un dossier `ports/` au sein du domaine.
- **Classes Abstraites :** Au lieu d'utiliser des `interfaces` TypeScript (qui disparaissent au runtime), tu dois définir les Ports comme des **`abstract class`**. Cela permet de les instancier et de les injecter via des factory functions ou des Context Providers React.
...
### A. Type Result (`src/lib/result/result.ts`)
...
### D. Conséquence pour la Présentation (React UI)

Les Composants React ou hooks personnalisés de la couche `presentation/` n'utilisent **plus de try/catch**. Ils inspectent simplement `result.success` pour décider du feedback utilisateur (ex: mise à jour d'un état de chargement ou d'erreur). Ils n'ont plus besoin d'importer les classes d'erreurs du domaine.