...
2. **Propose systématiquement la mise à jour du fichier `src/app/globals.css` (dans le bloc `@theme`)** si tu as inventé un nouveau token sémantique pour répondre au besoin du design.
...
<div className="bg-gray-50">
  <div className="bg-white rounded-xl border-gray-100">...</div>
  <button className="bg-indigo-600 hover:bg-indigo-700 rounded-md">Valider</button>
  <input className="border-gray-300 focus:ring-indigo-500 rounded-md" />
</div>
```
...
<div className="bg-background">
  <div className="bg-surface rounded-card border-subtle">...</div>
  <button className="bg-primary hover:bg-primary-hover rounded-btn text-primary-content">Valider</button>
  <input className="border-input focus:ring-primary focus:border-primary rounded-input" />
</div>
```

## 4. Contrat du Tailwind Config via @theme

Voici à quoi notre configuration cible ressemble dans `src/app/globals.css`. Appuie-toi sur cette logique pour nommer tes classes dans le JSX et configurer les tokens CSS :
```css
/* src/app/globals.css */
@import 'tailwindcss';
...
- Tu ajoutes à la fin de ta réponse : "Note: N'oubliez pas d'ajouter la variable css `--color-success: #22c55e;` dans le bloc `@theme` de votre fichier `src/app/globals.css` si elle n'y est pas déjà."